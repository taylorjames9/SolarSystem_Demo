...
public class ForceOnParticle : MonoBehaviour
{
	...
	public ForceField field_2;
	public ForceField field_3;
		...
		if (field_2 != null)
		{
			foreach (ParticleEmitter pe in pes)
			{
				...
				for (int i = 0; i < num; i++)
				{
					...
					if (!pe.useWorldSpace)
					{
						force = t.InverseTransformDirection(field_2.GetForce(t.TransformPoint(particles[i].position)));
					}
					else
					{
						force = field_2.GetForce(particles[i].position);
					}
					particles[i].velocity += force * factor;
				}
				pe.particles = particles;
...
//					if (!pe.useWorldSpace)
//					{
//						force = t.InverseTransformDirection(field_3.GetForce(t.TransformPoint(particles[i].position)));
//					}
//					else
//					{
//						force = field_3.GetForce(particles[i].position);
//					}
//					particles[i].velocity += force * factor;
//				}
//				pe.particles = particles;
